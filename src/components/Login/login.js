...
export default class Login extends React.Component {
  ...
  getAuthCode(){
    ...
    if (sendTime && new Date().getTime() - sendTime * 1 < 60 * 1000 ) {
      alert('60秒内仅能获取一次验证码，请稍后重试');
      return;
    }
    ...
    this.countDownFun = setInterval(() => {
      if (this.state.countDown === 0) {
        clearInterval(this.countDownFun);
        this.setState({ countDown: 60, showAuthCode: true });
      } else {
    ...
    return (
      <div className="logindiv1 shadow">
        ...
        <div className="form regf" onChange={this.onChange}>
          ...
          <div className="row">
            <input className="put mobile" value={regPhone} maxLength={11} name="regPhone" type="tel" placeholder="请输入手机号码"/>
          </div>
